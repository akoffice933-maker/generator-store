...
export default function ProductCard({ product }: { product: ProductListItem }) {
  const { segment, addToCart, favorites, toggleFavorite, compare, toggleCompare } = useStore();
  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-2xl border border-white/5 bg-[#111317] transition-all duration-300 hover:-translate-y-1 hover:border-white/10 hover:shadow-xl hover:shadow-black/40">
      <div className="relative aspect-square overflow-hidden bg-[#15171c]">
        ...
        <button
          ...
          className={cn(
            ...
            isFav ? "bg-[#E0561E] text-white" : "bg-black/50 text-gray-300 hover:text-white"
          )}
        >
          <Heart size={15} fill={isFav ? "currentColor" : "none"} />
        </button>
        {outOfStock && (
          <div className="absolute inset-x-0 bottom-0 bg-black/70 py-1 text-center text-xs font-semibold text-gray-300">
            Нет в наличии
          </div>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <Link href={`/catalog/${product.slug}`} className="mt-1 line-clamp-2 font-display text-sm font-semibold leading-snug hover:text-[#E0561E]">
          {product.name}
        </Link>
        ...
        <div className="mt-4 flex items-end justify-between">
          <div>
            {segment === "b2b" ? (
              <>
                <p className="text-[11px] text-gray-500 line-through">{formatPrice(retail)}</p>
                <p className="font-display text-lg font-bold text-[#E0561E]">{formatPrice(wholesale)}</p>
              </>
            ) : (
              <p className="font-display text-lg font-bold text-white">{formatPrice(retail)}</p>
            )}
          </div>
          <button
            ...
            className={cn(
              ...
              isCompared ? "border-[#E0561E] bg-[#E0561E]/10 text-[#E0561E]" : "border-white/10 text-gray-400 hover:text-white"
            )}
            title="Сравнить"
          >
            <GitCompare size={14} />
        ...
        <button
          ...
          className={cn(
            "mt-3 flex w-full items-center justify-center gap-2 rounded-full px-4 py-2.5 text-sm font-semibold transition-colors",
            outOfStock
              ? "cursor-not-allowed bg-white/5 text-gray-500"
              : "bg-[#E0561E] text-white hover:bg-[#F26128]"
          )}
        >
          <ShoppingCart size={15} />
          {outOfStock ? "Нет в наличии" : segment === "b2b" ? "Опт-заказ" : "В корзину"}
        </button>
      </div>
    </div>
  );
